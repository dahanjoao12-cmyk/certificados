...
export async function POST(request: NextRequest) {
  ...
  const summary = await processAlvaraImportRows(supabase, rows, mapping, user.id, true);

  return NextResponse.json({
    totalRows: summary.totalRows,
    alvarasCreated: summary.alvarasCreated,
    alvarasUpdated: summary.alvarasUpdated,
